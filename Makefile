...
SIZE := size
OBJCOPY := objcopy
CFLAG := -mcpu=cortex-m4 -mthumb -mfpu=fpv4-sp-d16 -mfloat-abi=hard -Idriver/st/ll/inc -Idriver/st/dev/inc -Idriver/cmsis/core -Idriver/cmsis/dsp/inc -Iuser/inc -Ilib/oled/inc -g -save-temps=obj -Wl,--gc-sections -DUSE_FULL_LL_DRIVER -Os -ffunction-sections -fdata-sections -specs=nano.specs -Ldriver/cmsis/dsp/lib -larm_cortexM4lf_math
BUILD_DIR := build/
BINARY_DIR := bin/
...
#@openocd -f interface/stlink-v2.cfg -f target/stm32g4x.cfg -c "adapter speed 1000; program bin/main.elf verify reset exit"
